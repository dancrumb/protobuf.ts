...
Deno.test("assertLongEquals", async (t) => {
  await t.step("0===0", () => {
    assertLongEquals(Long.fromValue(0), Long.fromValue(0), "0===0");
  });
  await t.step("1!==0", () => {
    assertThrows(() =>
      assertLongEquals(Long.fromValue(1), Long.fromValue(0), "1!==0")
    );
  });
  await t.step("0!==1", () => {
    assertThrows(() =>
      assertLongEquals(Long.fromValue(0), Long.fromValue(1), "0!==1")
    );
  });
});
